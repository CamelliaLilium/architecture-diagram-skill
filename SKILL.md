...
**声明顺序决定布局**: Mermaid 的 dagre 引擎按代码中节点的声明顺序排列。利用这一点：
- 按阅读顺序声明节点（用户在左/上，数据存储在右/下）
- 减少边交叉可提高读者理解准确率 30-40%

**隐形链接控制位置**: 用 `~~~` 创建不可见的连接来强制节点对齐：
```mermaid
A ~~~ B  %% A和B会水平对齐，但无可见连线
```

**间距调节**: 通过 init 配置微调节点间距：
```mermaid
...
当用户要求"帮我画出这个项目的架构图"时，按以下流程从代码分析生成：

1. **扫描入口**: 分析 imports/exports 识别模块依赖关系
2. **提取路由**: 解析 API 路由定义，映射到时序图
3. **解析数据模型**: 读取 ORM/Schema 定义，生成 ER 图
4. **识别分层**: 按目录结构识别架构层级（controller/service/repository）
5. **多视角输出**: 为同一系统生成多张互补的图：
   - **结构视角**: 系统总览/C4 Container 展示组件划分
   - **动态视角**: 时序图展示关键请求链路
   - **数据视角**: ER 图展示数据模型

关键原则：图中的服务名、端口、技术栈必须与代码实际一致，不使用占位符。
