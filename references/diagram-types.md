...
arc42 是一个架构文档框架，与 C4 模型思路相通且可配合使用。其核心章节直接对应架构图类型：

| arc42 章节 | 对应图型 | 内容 |
|-----------|---------|------|
| Context & Scope | C4 Context / 系统总览 | 系统边界和外部依赖 |
| Building Block View | C4 Container + Component | 静态分解：模块/容器/组件层级 |
| Runtime View | 时序图 / 状态图 | 关键场景的动态行为 |
| Deployment View | 部署架构 | 逻辑节点到物理节点的映射 |

**与 C4 的关系**: arc42 是文档框架，C4 是制图方法——两者互补。用 C4 画图，用 arc42 组织文档。

## 五环方法论

来自阿里技术团队的架构制图方法论：

1. **理解制图目标**: 准确、完整、清晰、一致、简洁
2. **找准受众关注点**: 研发看接口、运维看部署、安全看边界、产品看功能
3. **自顶向下逐层描述**: 运用分而治之和金字塔原理控制复杂度
4. **使用多种架构视图**: 结构视图、动态视图、部署视图相互补充
5. **遵循规范和最佳实践**: 正交性、高内聚、低耦合、隔离变化
